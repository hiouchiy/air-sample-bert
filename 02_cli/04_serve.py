"""ModernBERT -> Databricks Model Serving — control-plane script (not a GPU job).

Creates/updates a GPU serving endpoint for the @champion UC model and sends a test query. Run it
locally (it only needs mlflow), not via the AI Runtime CLI:
