#!/usr/bin/env bash
# One-time Unity Catalog setup for the ModernBERT AI Runtime demo.
#
...
Done. Now point the demo at this catalog by either:
  ...
  - prefixing the command in each 02_cli/*.yaml, e.g.:
      command: UC_CATALOG=$CATALOG python \$CODE_SOURCE_PATH/02_cli/01_finetune_singlegpu.py
Then run:  COPYFILE_DISABLE=1 air run --file 02_cli/finetune_singlegpu.yaml --watch --profile $PROFILE
EOF
