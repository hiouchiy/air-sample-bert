...
Done. Now point the demo at this catalog by either:
  ...
  - prefixing the command in each air/*.yaml, e.g.:
      command: UC_CATALOG=$CATALOG python \$CODE_SOURCE_PATH/src/01_finetune_singlegpu.py
Then run:  COPYFILE_DISABLE=1 air run --file air/finetune_singlegpu.yaml --watch --profile $PROFILE
EOF
